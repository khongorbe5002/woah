...
if __name__ == "__main__":
    ...
    mp.Process(target=run_sensor_process, args=(shared_sensor_data, sensor_lock), daemon=True).start()
    ...
    while True:
        ...
        now = time.time()
        _cleanup_tracked_objects(now)

        detections_for_scene = []
        ...
        for r in results:
            for b in r.boxes:
                ...
                # ✅ FIXED DISTANCE SAMPLING
                region = sensor_data[max(0,gy-1):min(8,gy+2), max(0,gx-1):min(8,gx+2)]
                valid = region[region > 0]
                dist = int(np.median(valid)) if len(valid) > 0 else 0
                ...
                detections_for_scene.append({"label": display_label})

                cv2.rectangle(frame, (x1,y1), (x2,y2), (0,255,0), 2)

                # ✅ FIXED TRACKING + ALERT LOGIC
                if 0 < dist < 1000 and current_mode != 4:
                    ...
                    if "last_alert" not in obj or now - obj["last_alert"] > ALERT_REMINDER_SECONDS:
        ...
        if current_mode == 4:
            describe_scene(detections_for_scene)
